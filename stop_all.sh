...
if is_windows && command -v powershell.exe >/dev/null 2>&1; then
  ...
  powershell.exe -Command "\$p = Get-CimInstance Win32_Process | Where-Object { \$_.CommandLine -match 'main.py' }; if (\$p) { \$p | ForEach-Object { Stop-Process -Id \$_.ProcessId -Force } }" && echo "Stopped Flask" || true
else
